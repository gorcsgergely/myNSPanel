...
void EasyNex::readCommand(){
  ...
  switch(_cmd1){
    ...
    case 0xA0: //Cover position changed: 0x3D 0x09 0xA0 COVER_NUMBER 0x00 0x00 0x00 COVER_POSITION 0x00 0x00 0x00
      selectedCover = _serial->read();  
      dummy = _serial->read();  
      dummy = _serial->read();
      dummy = _serial->read();
      coverPosition = _serial->read();
      dummy = _serial->read();  
      dummy = _serial->read();
      dummy = _serial->read();
